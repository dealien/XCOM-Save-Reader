...
class MainMenu(ctk.CTkFrame):
    def __init__(self, parent, controller):
        ...
        label = ctk.CTkLabel(self, text="Main Menu", font=ctk.CTkFont(size=20, weight="bold"))
        label.pack(pady=20)

        load_button = ctk.CTkButton(self, text="Load Save File", command=self.load_save)
        load_button.pack(pady=10)

        self.soldiers_button = ctk.CTkButton(self, text="View Soldiers", state="disabled", command=self.show_soldiers)
        self.soldiers_button.pack(pady=10)

        self.bases_button = ctk.CTkButton(self, text="View Bases", state="disabled", command=self.show_bases)
